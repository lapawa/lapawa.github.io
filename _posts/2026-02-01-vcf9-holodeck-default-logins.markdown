---
layout: post
title:  "VCF9 Holodeck List of default logins"
date:   2026-02-01 12:00:00
categories: vcf
---
| fqdn                          | service(s) | description                     | username                    |
| ----------------------------- | ---------- | ------------------------------- | --------------------------- |
| holorouter                    | ssh        | Holodeck entry point appliance  | root                        |
| vcfinstaller-a.site-a.vcf.lab | https      | VCF Installer web frontend      | admin@local                 |
| esxi-01a.site-a.vcf.lab       | ssh+https  | Nested ESXi Systems             | root                        |
| vc-mgmt-a.site-a.vcf.lab      | https      | vSphere Client                  | administrator@vsphere.local |
| vc-mgmt-a.site-a.vcf.lab      | ssh        | vCenter Server appliance        | root                        |
| nsx-mgmt-a.site-a.vcf.lab     | https      | NSX Manager web frontend        | admin                       |
| ops-a.site-a.vcf.lab          | https      | VCF Operations web frontend     | admin                       |
| sddcmanager-a.site-a.vcf.lab  | https      | VCF SDDC Manager web frontend   | admin                       |
| sddcmanager-a.site-a.vcf.lab  | ssh        | VCF SDDC Manager appliance      | vcf                         | 
| auto-a.site-a.vcf.lab         | ssh        | Automation Appliance            | vmware-system-user          |
| 

